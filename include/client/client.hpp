...
template <class Response, class Request>
boost::outcome_v2::
    result<std::unique_ptr<http_response<Response>>> inline send_https(
  ...
  auto resp = std::make_unique<http_response<Response>>(std::move(ssl_stream));
...
template <class Response, class Request>
boost::outcome_v2::result<std::unique_ptr<http_response<Response>>> inline send(
